...
describe('Cache - Handlers - cacheStoreObject', function () {
  ...
  beforeEach(function () {
    doubles = {
      ...
      cacheModuleStub: {
        setObject: sandbox.stub()
      },
    };
    proxyHandler = proxyquire('../../../../dist/cache/handlers/cacheStoreObject.js', {
  ...
  describe('returned function', function () {
    let handler;
    beforeEach(function () {
      doubles.cacheModuleStub.setObject.returns(Promise.resolve(true));
    ...
    it('should call the "setObject" property of the provided cache object once', function () {
      handler({});
      assert.isTrue(doubles.cacheModuleStub.setObject.calledOnce);
    });

    it('should call the "setObject" property of the provided cache object with 2 arguments', function () {
      handler({});
      assert.strictEqual(doubles.cacheModuleStub.setObject.args[0].length, 2);
    });
    ...
    describe('first argument', function () {
      it('should be the "key" property of the first argument provided to this function', function () {
        const key = 'myKey';
        handler({ key });
        assert.strictEqual(doubles.cacheModuleStub.setObject.args[0][0], key);
      });
    });
    ...
    describe('second argument', function () {
      it('should be the "value" property of the first argument provided to this function', function () {
        const value = 'myValue';
        handler({ value });
        assert.strictEqual(doubles.cacheModuleStub.setObject.args[0][1], value);
      });
    });

    describe('if the "setObject" property of the provided cache object returns a promise that resolves', function () {
      describe('if the promise resolves with TRUE', function () {
        describe('if an "ack" function was provided', function () {  
          it('should call it once', function () {
            ...
            doubles.cacheModuleStub.setObject.returns(returnPromise);
            handler({}, ack);
            return(
              returnPromise
              .then(function () {
      ...
      describe('if the promise resolves with FALSE', function () {
        ...
        it('should call the error() function of the logger with a custom error message in the "message" property', function () {
          ...
          return(
            new Promise(function (resolve, reject) {
              const key = 'myKey';
              const value = 'myValue';
              const errorMsg = `Failed to store object in the cache with key "${key}" and value ${JSON.stringify(value)}`
              handler({ key, value });
              global.setTimeout(function () {
        ...
        describe('if an "ack" message was provided', function () {
          it('should call it once', function () {
            const ack = sandbox.stub();
            const returnPromise = Promise.resolve(false);
            doubles.cacheModuleStub.setObject.returns(returnPromise);
    ...
    describe('if the "setObject" property of the provided cache object returns a promise that rejects', function () {
      it('should call the error() function of the logger once', function () {
      ...
      describe('if an "ack" function was provided', function () {
        ...
        it('should call it with Error object', function () {
          const ack = sandbox.stub();
          const testError = new Error('test error message');
          const returnPromise = Promise.reject(testError);
          doubles.cacheModuleStub.setObject.returns(returnPromise);
